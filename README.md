...
# Note:
Sometimes, the server will crash when launching it for the first time or when updating it. If it does crash, just try running it a few more times until it starts. The crash will not happen again after is starts up successfully once unless you are launching a different version of the server.
